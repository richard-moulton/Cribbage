...
#
# Description : A player that uses first visit Monte Carlo to learn the Q values
#               for different states. A minor modification of QLearner.
#
...
class Monty(Player):
    def __init__(self, number, verbose=False, hiddenlayers=(50, 30, 15), alpha=0.1, epsilon=0.05, gamma=0.3,
                 filename='CarloMcMonty'):
        super().__init__(number, verbose)
        self.alpha = alpha
        # Note: epislon unused since we use softmax decision making
        self.epsilon = epsilon
        self.gamma = gamma
        self.name = 'Carlo McMonty'
        self.hiddenlayers = hiddenlayers
        self.throwingstate = []
        self.episodeStates = []
        self.episodeActions = []
        self.episodeReturns = []
        self.noActionIndex = 5
        self.goActionIndex = 4
    ...
    def playCard(self, gameState):
        ...
        if self.currentAction == self.goActionIndex:
            ...
            cardPlayed = self.playhand.pop(self.currentAction)
    ...
    def explainThrow(self,numCards,gameState):
        print("Carlo McMonty ({}) has not implemented explainThrow".format(self.number))
    ...
    def updateBrain(self, gameState):
        if self.episodeActions:
            ...
            for i in reversed(range(len(self.episodeStates))):
                ...
                G = Gt[i]
                Y[i][self.episodeActions[i]] = Gt[i]
...
if __name__ == "__main__":
    ...
    player1 = Monty(1, False, alpha=0.9, epsilon=0.1, gamma=1)
    ...
    fig.savefig("CarloMcMonty_a" + str(player1.alpha) +
